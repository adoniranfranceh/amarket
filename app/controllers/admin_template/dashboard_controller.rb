class AdminTemplate::DashboardController < AdminTemplateController
  def index
    choose_date_request
    calculate_info
  end
  ...
  def calculate_info
    calculate_sales_info
    calculate_customer_info
    calculate_product_info
  end

  def calculate_sales_info
    sales_completed = @sales.where(status: 'completed')
    @total = sales_completed.sum(:total_price)
  end

  def calculate_customer_info
